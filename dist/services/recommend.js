...
import { isCloudModel, findLocalInstalledModel, extractModelScore, getOllamaModelUrl, } from "../utils/model.js";
const BASE_SCORE = 50;
const INSTALLED_BONUS = 5;
const TIER_DOWNGRADE_PER_LEVEL = 10;
const TASK_MATCH_BONUS = { tools: 15, thinking: 10, vision: 40 };
const TASK_MATCH_PENALTY = { vision_missing: -100 };
const AGENTIC_BONUS = { tools: 25, thinking: 15 };
const FAST_TIER_DOWNGRADE_PER_LEVEL = 30;
const GENERAL_TIER_DOWNGRADE_PER_LEVEL = 15;
function scoreCategory(benchmarks, rawName, categories) {
    if (!benchmarks)
        return null;
    const rows = benchmarks.rows.filter((r) => categories.includes(r.category?.toLowerCase() ?? ""));
    if (rows.length === 0)
        return null;
    let total = 0;
    let count = 0;
...
const TASK_SCORERS = {
    coding({ candidate, caps }) {
        ...
        if (caps.includes("tools"))
            bonus += TASK_MATCH_BONUS.tools;
        if (caps.includes("thinking"))
            bonus += TASK_MATCH_BONUS.thinking;
    ...
    agentic({ candidate, caps }) {
        ...
        if (caps.includes("tools"))
            bonus += AGENTIC_BONUS.tools;
        if (caps.includes("thinking"))
            bonus += AGENTIC_BONUS.thinking;
        return { score: bonus, reason };
    },
...
const DEFAULT_SCORER = ({ candidate }) => ({
...
export async function recommendModel(options) {
    ...
    const candidates = enrichedModels.filter((m) => {
        ...
        const ctx = m.model_info?.context_length ||
            ...
            (m.model_info?.[`${m.details?.family}.context_length`] ?? 0);
