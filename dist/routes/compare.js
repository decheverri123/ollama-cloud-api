...
export const handleCompare = withError(async (_req, res, parsedUrl) => {
    ...
    const comparedModels = await Promise.all(modelNames.map(async (name) => {
        ...
        return {
            ...
            parameter_size: enriched.parameter_size || enriched.details?.parameter_size,
    ...
    sendJson(res, 200, {
        compared_models: comparedModels.map((model) => ({
            ...
            parameter_size: model.parameter_size || model.details?.parameter_size,
