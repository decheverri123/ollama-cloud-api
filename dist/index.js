...
export const benchmarkCache = new Map();
 ...
 */
export function calculateTierFromPricing(inputCost, outputCost) {
    if (inputCost > 2.0 || outputCost > 5.0)
        return 4; // Extra High
    if (inputCost >= 0.8 || outputCost >= 3.2)
        return 3; // High
    if (inputCost >= 0.25 || outputCost >= 1.0)
        return 2; // Medium
    return 1; // Low
}
/**
...
export function isCloudModel(model) {
    return Boolean(model.remote_host ||
        model.remote_model ||
 ...
 */
export async function fetchModelBenchmarks(modelName) {
    ...
    for (const url of urls) {
        try {
            ...
            const benchMarkdownMatch = html.match(/#+\s*Benchmarks[\s\S]*?(\|[\s\S]*?\|(?:\r?\n\s*\r?\n|(?=\s*#)|$))/i);
 ...
 */
export function getKnownModelTier(modelName) {
 ...
 */
export async function fetchModelUsage(modelName) {
    ...
    for (const url of urls) {
        try {
            ...
            const usageMatch = html.match(/Usage<\/div>\s*<div[^>]*>[\s\S]*?<span[^>]*class="[^"]*min-w-0 break-words[^"]*"[^>]*>([^<]+)<\/span>/i);
 ...
 */
export async function fetchLiveCloudCatalog() {
    ...
    try {
        ...
        const html = await res.text();
        const matches = Array.from(html.matchAll(/<a[^>]*href="\/library\/([^"]+)"[^>]*>([\s\S]*?)<\/a>/gi));
        const models = [];
        const seen = new Set();
 ...
 */
export function findLocalInstalledModel(cloudSlug, localModels) {
...
async function getEnrichedModelData(modelName, verbose = true, includeBenchmarks = false) {
 ...
 */
function applyFiltersAndSort(models, searchParams) {
    let result = [...models];
 ...
 */
export function extractModelScore(scores, modelName) {
    ...
    let bestCandidateVal = null;
    let bestScoreNum = -1;
    for (const [col, val] of Object.entries(scores)) {
        if (val === null || val === undefined)
            continue;
        const colTokens = normalizeTokens(col);
        const colCore = colTokens.filter((t) => ![
            "non",
            "think",
            "thinking",
            "high",
        ...
        const modCore = modTokens.filter((t) => !["model", "cloud", "latest"].includes(t));
        const matchesAllCol = colCore.length > 0 && colCore.every((t) => modCore.includes(t));
        const matchesAllMod = modCore.length > 0 && modCore.every((t) => colTokens.includes(t));
        ...
        if (matchesAllCol || matchesAllMod || keyTokensMatch) {
            if (typeof val === "number") {
                if (col.toLowerCase().includes("high") ||
 ...
 */
function computeLeaderboard(benchmarkDataList, usageMap) {
 ...
 */
async function recommendModel(options) {
    ...
    const enrichedList = await Promise.all(liveCatalog.map(async (catModel) => {
    ...
    const scoredCandidates = candidates.map((m) => {
        ...
        if (task === "coding") {
            ...
            const codingRows = rows.filter((r) => r.category?.toLowerCase() === "coding" ||
            ...
            if (caps.includes("thinking"))
                score += 10;
            if (!reason)
                reason = `Supports coding with capabilities [${caps.join(", ")}] at tier ${m.usage}`;
        }
        else if (task === "agentic") {
            const rows = m.benchmarks?.rows || [];
            const agenticRows = rows.filter((r) => r.category?.toLowerCase() === "agentic" ||
            ...
            if (caps.includes("thinking"))
                score += 15;
            if (!reason)
                reason = `Agent-ready model with tool calling at tier ${m.usage}`;
        }
        else if (task === "vision") {
    ...
    paths: {
        "/api/show-cloud": {
            get: {
                ...
                description: "Lists all Ollama Cloud models live from Ollama's catalog, enriched with local installed status (installed: true/false), parameters, template, capabilities, model_info, and numeric usage tiers (1=Low, 2=Medium, 3=High, 4=Extra High).",
                parameters: [
                    {
                        name: "model",
                        in: "query",
                    ...
                    },
                    {
                        name: "capability",
                        in: "query",
                        description: "Filter models possessing required capabilities (e.g. tools,vision,thinking).",
        ...
        "/api/recommend": {
            get: {
                ...
                description: "Recommends the best cloud model matching task requirements (coding, agentic, vision, fast, cheap) and usage limits.",
                parameters: [
                    {
                        name: "task",
                        in: "query",
            ...
            post: {
                ...
                description: "Same recommendation engine as the GET endpoint, accepting parameters in a JSON request body.",
                requestBody: {
                    required: false,
                    content: {
                        "application/json": {
        ...
        "/api/chat": {
            post: {
                ...
                description: "Forwards a standard Ollama chat request to the upstream Ollama server. If 'model' is omitted and 'task' is provided, the best matching cloud model is selected automatically. Response includes the model's usage tier via the X-Model-Usage-Tier header.",
                requestBody: {
                    required: true,
                    content: {
                        "application/json": {
        ...
        "/api/generate": {
            post: {
                ...
                description: "Forwards a standard Ollama generate request to the upstream Ollama server. If 'model' is omitted and 'task' is provided, the best matching cloud model is selected automatically. Response includes the model's usage tier via the X-Model-Usage-Tier header.",
                requestBody: {
                    required: true,
                    content: {
                        "application/json": {
        ...
        "/api/leaderboard": {
            get: {
                ...
                description: "Ranks all models by category (Coding, Agentic, Vision) based on scraped benchmark scores.",
                parameters: [
                    {
                        name: "category",
                        in: "query",
        ...
        "/api/compare": {
            get: {
                ...
                description: "Compares 2 or more cloud models side-by-side (context length, parameter sizes, usage tiers, and benchmark deltas).",
                parameters: [
                    {
                        name: "models",
                        in: "query",
        ...
        "/api/overview": {
            get: {
                ...
                description: "Dashboard metrics covering live catalog size, installed vs uninstalled counts, tier distributions, capabilities breakdown, context lengths, and benchmark coverage.",
                responses: {
                    "200": {
        ...
        "/api/benchmarks": {
            get: {
                ...
                description: "Retrieves scraped benchmark comparison results from Ollama's library page (Coding, Agentic, Vision, Math).",
                parameters: [
                    {
                        name: "model",
                        in: "query",
                        description: "Model name (e.g. glm-5.3-flash:cloud). If omitted, returns all cloud models with benchmarks.",
        ...
        "/api/show-cloud/grouped": {
            get: {
                ...
                description: "Returns all cloud models categorized into 1_low, 2_medium, 3_high, and 4_extra_high.",
                responses: {
                    "200": {
        ...
        "/api/tags-cloud": {
            get: {
                ...
                description: "Fast tags-compatible endpoint returning only cloud models with usage numbers (1-4) and installed status.",
                parameters: [
                    {
                        name: "installed",
                        in: "query",
        ...
        "/api/ps-cloud": {
            get: {
                ...
                description: "Lists currently running/loaded Ollama cloud models along with their usage tier.",
                responses: {
                    "200": {
        ...
        "/api/cache/status": {
            get: {
                ...
                description: "View in-memory usage, benchmark, and catalog cache size, TTL, entries, ages, and time-to-expiry.",
                responses: {
                    "200": {
...
<html>
  ...
  <body>
    ...
    if (pathname === "/api/overview" || pathname === "/overview") {
        try {
            ...
            await Promise.all(liveCatalog.map(async (catModel) => {
                const name = catModel.name;
                const u = await fetchModelUsage(name);
                const key = u === 1 ? "1_low" : u === 2 ? "2_medium" : u === 3 ? "3_high" : "4_extra_high";
                ...
                if (!localMatch) {
                    ...
                    const ctx = localMatch.model_info?.context_length ||
    ...
    if (pathname === "/api/leaderboard" || pathname === "/leaderboard") {
        try {
            ...
            if (requestedCat && leaderboards[requestedCat]) {
                res.writeHead(200, { "Content-Type": "application/json" });
                return res.end(JSON.stringify({ category: requestedCat, leaderboard: leaderboards[requestedCat] }, null, 2));
            }
            res.writeHead(200, { "Content-Type": "application/json" });
            return res.end(JSON.stringify({
    ...
    if (pathname === "/api/compare" || pathname === "/compare") {
        ...
        try {
            const comparedModels = await Promise.all(modelNames.map(async (name) => {
                    ...
                    if (score === null) {
                        for (const other of comparedModels) {
                            if (other.name !== m.name && other.benchmarks?.rows) {
                                const foundRow = other.benchmarks.rows.find((r) => r.benchmark === bName);
                                if (foundRow) {
            ...
            return res.end(JSON.stringify({
                compared_models: comparedModels.map((m) => ({
                    ...
                    context_length: m.model_info?.context_length ||
                        m.model_info?.[`${m.details?.family}.context_length`],
                })),
    ...
    if (pathname === "/api/benchmarks" || pathname === "/benchmarks") {
        ...
        try {
            const liveCatalog = await fetchLiveCloudCatalog();
            const benchmarkResults = await Promise.all(liveCatalog.map(async (catModel) => {
                const name = catModel.name;
    ...
    if (pathname === "/api/cache/status") {
        ...
        const usageEntries = Array.from(usageCache.entries()).map(([model, entry]) => ({
            ...
            expires_in_seconds: Math.max(0, Math.round((CACHE_TTL_MS - (now - entry.timestamp)) / 1000)),
        }));
        const benchmarkEntries = Array.from(benchmarkCache.entries()).map(([model, entry]) => ({
            model,
    ...
    if (pathname === "/api/tags-cloud" || pathname === "/tags-cloud") {
        try {
            ...
            const localCloudModels = rawLocalModels.filter(isCloudModel);
            const enrichedTags = await Promise.all(liveCatalog.map(async (catModel) => {
                const name = catModel.name;
                const localMatch = findLocalInstalledModel(name, localCloudModels);
    ...
    if (pathname === "/api/ps-cloud" || pathname === "/ps-cloud") {
        try {
            ...
            const enrichedRunning = await Promise.all(cloudRunning.map(async (m) => {
    ...
    if (pathname === "/api/show-cloud/grouped" ||
        ...
        req.on("end", async () => {
            try {
                ...
                res.end(JSON.stringify({
                    ...
                    hint: err.name === "TypeError" || /fetch failed/i.test(err.message || "")
...
async function handleShowCloud(_req, res, payload) {
...
async function handleShowCloudAll(_req, res, parsedUrl, grouped = false, includeBenchmarks = false) {
    try {
        const [rawLocalModels, liveCatalog] = await Promise.all([
            fetchOllamaTags(),
            fetchLiveCloudCatalog(),
